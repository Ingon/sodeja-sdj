	...
	public List<SchemeExpression> tokenize(Reader baseReader) throws IOException {
		...
		for(Character ch = read(reader);ch != null;ch = read(reader)) {
			...
			if(ch == '(') {
				...
				((Combination) stack.peekLast()).parts.add(listToken);
			...
			if(ch == ')') {
				...
				if(currentPrimitiveToken.length() != 0) {
					theToken.parts.add(new Symbol(currentPrimitiveToken.toString()));
			...
			if(ch == '\'') {
				Combination listToken = new Combination();
				listToken.parts.add(new Symbol("quote"));
		...
		return ((Combination) (stack.pollLast())).parts;
	}
